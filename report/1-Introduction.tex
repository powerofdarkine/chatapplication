...
\begin{itemize}
    ...
    \item \textbf{HTTP}: The Hypertext Transfer Protocol (HTTP) serves as the foundation for the Client–Server portion of the system. It follows a stateless request–response model in which clients send requests and servers return corresponding responses. HTTP headers and cookies enable additional stateful functionality such as session persistence and user authentication. Furthermore, this project uses periodic HTTP polling (short-poll) to simulate near real-time communication over standard HTTP; the server responds immediately and the browser polls at regular intervals.
...
\begin{itemize}
    ...
    \item \textbf{Polling (short-poll) Mechanism for Real-Time Interaction}: This project implements periodic short-polling from the browser to the backend: the client regularly sends requests at fixed intervals and the server returns any available events or messages immediately. While short-polling can increase request frequency compared to long-polling, it is simpler to implement in a thread-per-request server model and avoids holding worker threads waiting on I/O; the frontend can tune the polling interval to balance latency and overhead.
