...
\subsubsection{Client-Side Long-Polling}
\label{subsec:appendix_polling}
The \texttt{pollMessages} function implements the client-side of the long-polling mechanism. It periodically calls the \texttt{/api/p2p-receive} endpoint and processes the returned messages, including handling the special \texttt{CLOSE} message type to gracefully end a session.
...
\begin{longtable}{|p{0.1\linewidth}|p{0.25\linewidth}|p{0.55\linewidth}|}
    ...
    \hline
    POST & \texttt{/api/broadcast-peer} & Long-polling endpoint for receiving real-time peer events (e.g., `peer-joined`, `peer-left`). \\
    \hline
...
\end{lstlisting}
\textbf{Analysis:} The logs validate the HTTP-to-TCP-to-HTTP message bridge. The \texttt{/api/p2p-send} call from \texttt{user1}'s browser results in a \texttt{CHAT} message being sent over the direct TCP link. \texttt{user2}'s daemon receives it, places it in the user-specific queue, and the message is retrieved by \texttt{user2}'s browser via a long-poll to \texttt{/api/p2p-receive}. This matches the logic in Listings~\ref{lst:p2p_send} and \ref{lst:p2p_receive}.
