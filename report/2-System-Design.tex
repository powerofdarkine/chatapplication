...
\hline
\textbf{Frontend (Browser)} \newline (\texttt{chat.js}) & Provides the graphical user interface for chat interactions. Initiates all user actions via asynchronous HTTP requests, including long-polling for peer updates and message retrieval. & HTTP & Proxy Server \\
\hline
...
\begin{enumerate}
    ...
    \item Each connected client, which is continuously long-polling the \texttt{/api/broadcast-peer} endpoint, receives this update and dynamically refreshes its user interface to display the newly online peer.
\end{enumerate}

This mechanism, built upon long-polling, allows all clients to maintain near real-time awareness of network membership without relying on WebSocket connections.
...
and transmits it directly over the active TCP socket.
    ...
    \item User B’s browser, which continuously long-polls the \texttt{/api/p2p-receive} endpoint, retrieves the message from the queue and renders it in the chat window.
\end{enumerate}
